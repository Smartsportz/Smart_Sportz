...
def _verify_otp(challenge_id: str, code: str) -> dict:
    ...
    if not code_matches and challenge.get("provider") == "twilio_verify":
        provider_check = check_sms_otp(challenge.get("target", ""), code)
        code_matches = provider_check.ok
        challenge["providerMessage"] = provider_check.message
    if not code_matches:
...
def forgot_password_start(payload: ForgotPasswordStartRequest):
    ...
    if user:
        code = generate_otp(4)
        delivery = send_email_otp(target, code)
        challenge_id = _store_otp_challenge("password_reset", {"userId": user["id"]}, "email", target, code, "email")
        log(target, "password_reset_otp_sent", "auth", user["id"], delivery.message)
        return _challenge_response(challenge_id, "email", target, _generic_recovery_message(target))
    fake_challenge_id = _store_otp_challenge("password_reset_missing", {"email": target}, "email", target, generate_otp(4), "local")
...
def signup_verify(payload: SignupVerifyRequest):
    ...
    phone_verified = 1 if challenge["channel"] == "sms" else 0
    execute(
