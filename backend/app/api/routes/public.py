...
def home():
    return ok({
        ...
        "featuredTournaments": rows("SELECT * FROM tournaments LIMIT 3"),
...
def tournaments():
    return ok([attach_cities(item) for item in rows("SELECT * FROM tournaments ORDER BY name")])
...
@router.get("/sports")
def sports():
    return ok(rows("SELECT * FROM sports ORDER BY name"))
...
def sport_detail(slug: str):
    ...
    sport["groups"] = {
        "upcoming": [item for item in items if item["status"] in ("Registration Open", "Upcoming")],
...
@router.get("/teams")
def teams():
    return ok(rows("SELECT * FROM teams ORDER BY rating DESC"))
...
def cms(content_type: str):
    return ok(rows("SELECT * FROM cms_content WHERE lower(type) = lower(?) AND published = 1", (content_type,)))
